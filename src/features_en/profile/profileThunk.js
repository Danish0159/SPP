...
export const updateProfileThunkEn = async ({ profilePhoto, name, email, phoneNumber, role_en, role_ar, category_en, category_ar, subCategory_en, subCategory_ar, companyName, companyAbout, companyVision, companyMission, id }, thunkAPI) => {
  try {
    const resp = await customFetchProfile.patch(`/profile/updateprofileen/${id}`, { profilePhoto, name, email, phoneNumber, role_en, role_ar, category_en, category_ar, subCategory_en, subCategory_ar, companyName, companyAbout, companyVision, companyMission });
    if (checkStatus(resp)) { return thunkAPI.rejectWithValue(resp.data.message); }
    return resp.data;
  } catch (error) {
    const message = checkError(error);
...
export const reviewProjectThunkEn = async ({ name, title, stars, phoneNumber, review, profileId, projectId }, thunkAPI) => {
  try {
    ...
    const resp = await axios.patch(`https://mahnty.herokuapp.com/api/profile/reviewen/${profileId}/${projectId}`, { name, title, stars, phoneNumber, review });
    if (checkStatus(resp)) { return thunkAPI.rejectWithValue(resp.data.message); }
    return resp.data;
  } catch (error) {
    const message = checkError(error);
